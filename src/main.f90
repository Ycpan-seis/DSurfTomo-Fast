...
!       2019/06/07, Song Luo, 1. redefined "goxd,gozd" as the true left top point
!                             2. calculated residuals of the final model
!                             3. delete variable "maxvp" in the function "synthetic"
!       2019/07/27, Song Luo, 1. revised a bug by initializing nrc1 = 0 and nsrc1 = 0
!       2020/11/22, Song Luo, I change "goxd,gozd" back to the original reference point
!       2026.2, Yichen Pan, parallel processing is performed when calling the FMM to calculate the travel time field.
!       2026/4/14, Yichen Pan, resolve integer overflow, enabling large-scale data processing. 
...
program SurfTomo
  ...
  integer, dimension(:,:,:), allocatable    ::  countsurf ! count index for parallel calculation, Yichen, 26.2.14
  ...
  integer istop
  integer itnlim
  integer*8 lenrw,leniw ! Yichen
  integer nars
  integer*8  nar,nar_tmp ! Yichen
  ...
  real averdws
  real maxnorm
  real threshold,threshold0,q25,q75
  real rough1
...
!  real,allocatable,dimension(:)::modsig
  ...
  if(kmaxRc.gt.0)then
    ...
    write(*,'( 50f10.3)')(tRc(i),i=1,kmaxRc)
    write(66,*)'Rayleigh wave phase velocity used,periods:(s)'
    write(66,'( 50f10.3)')(tRc(i),i=1,kmaxRc)
  endif
  read(10,*)kmaxRg
  if(kmaxRg.gt.0)then
    ...
    write(*,'( 50f10.3)')(tRg(i),i=1,kmaxRg)
    write(66,*)'Rayleigh wave group velocity used,periods:(s)'
    write(66,'( 50f10.3)')(tRg(i),i=1,kmaxRg)
  endif
  read(10,*)kmaxLc
  if(kmaxLc.gt.0)then
    ...
    write(*,'( 50f10.3)')(tLc(i),i=1,kmaxLc)
    write(66,*)'Love wave phase velocity used,periods:(s)'
    write(66,'( 50f10.3)')(tLc(i),i=1,kmaxLc)
  endif
  read(10,*)kmaxLg
  if(kmaxLg.gt.0)then
    ...
    write(*,'( 50f10.3)')(tLg(i),i=1,kmaxLg)
    write(66,*)'Love wave group velocity used,periods:(s)'
    write(66,'( 50f10.3)')(tLg(i),i=1,kmaxLg)
  endif
  read(10,*)ifsyn
  read(10,*)noiselevel
  read(10,*) threshold0
...
!  read(10,*) numrand
  ...
  allocate(countsurf(nrc,nsrc,kmax),stat=checkstat)
  if(checkstat > 0)then
    write(6,*)'error with allocate'
  endif
    ...
    if(err.eq.0) then
      if(line(1:1).eq.'#') then
        ...
        if(wavetp.eq.1.and.veltp.eq.1) knum=kmaxLc+kmaxRg+&
          kmaxRc+period
        if(knum.ne.knumo) then  ! ycpan,if knum change,reset istep
          istep=0
          istep2=istep2+1
        endif
        istep=istep+1  
        ...
        countsurf(istep1,istep,knum)=dall ! Yichen, 26.2.23
...
!  allocate(modsig(maxvp))
  ...
  if(checkstat > 0)then
    write(6,*)'error with allocate:  integer col' !modified by ycpan,2024/9/4
  endif
  ...
  read(10,*) (depz(i),i=1,nz) ! read all depth from the first row of MOD to depz
  do k = 1,nz
    do j = 1,ny
  ...
  if (ifsyn == 1) then
    ...
    call synthetic(nx,ny,nz,vsftrue,obst,&
  ...
  do iter = 1,maxiter
    ...
    call CalSurfG_parallel(nx,ny,nz,maxvp,vsf,iw,rw,col,dsyn,&
      ...
      tRc,tRg,tLc,tLg,wavetype,igrt,periods,depz,minthk,&
      scxf,sczf,rcxf,rczf,nrc1,nsrc1,kmax,&
      nsrc,nrc,nar,writepath,dall,countsurf,nthreads)

    do i = 1,dall
      !print *,"cbst,obst,dsyn",i,cbst(i),obst(i),dsyn(i) ! Yichen
    ...
    ! Yichen, calculate the roughness of the model
      rough1=0.0
      do k=1,nz-1
         do j=1,ny-2
            do i=1,nx-2
    ...
    !do i =1,dall
    !   cbst(i)=cbst(i)/datweight(i) ! ycpan, change back to the true data
    !enddo
    ...
    write(*,'(a,f8.1,a,f8.2,a,f9.6)'),'mean,std_devs and rms of &
      residual: ',mean*1000,'ms ',1000*std_devs,'ms ',&
      dnrm2(dall,cbst,1)/sqrt(real(dall))
    write(*,'(a,f12.4)') 'Roughness of the model              ', rough1
    write(66,'(i2,a)'),iter,'th iteration...'
    write(66,'(a,f7.3)'),'weight is:',weight
    write(66,'(a,f8.1,a,f8.2,a,f9.6)'),'mean,std_devs and rms of &
      residual: ',mean*1000,'ms ',1000*std_devs,'ms ',&
      dnrm2(dall,cbst,1)/sqrt(real(dall))
    write(66,'(a,f12.4)') 'Roughness of the model              ', rough1
    ...
    do k=1,nz
      do j=1,ny
        write(34,'(100f10.4)') (vsf(i,j,k),i=1,nx)
      enddo
    enddo
    write(34,*)',OUTPUT DWS AT ITERATION',iter
    do k=1,nz-1
      do j=2,ny-1
        write(34,'(100f10.4)') (norm((k-1)*(ny-2)*(nx-2)+(j-2)*(nx-2)+i-1),i=2,nx-1)
      enddo
    enddo
    ...
    do k=1,nz
      do j=1,ny
        do i=1,nx
          write(64,'(5f10.4)') gozd+(j-2)*dvzd,goxd-(i-2)*dvxd,depz(k),vsf(i,j,k)
        enddo
      enddo
    enddo
    close(64)
  ...
  ! call synthetic(nx,ny,nz,vsf,dsyn,&
  ! goxd,gozd,dvxd,dvzd,kmaxRc,kmaxRg,kmaxLc,kmaxLg,&
  ! tRc,tRg,tLc,tLg,wavetype,igrt,periods,depz,minthk,&
  ! scxf,sczf,rcxf,rczf,nrc1,nsrc1,kmax,&
  ! nsrc,nrc,0.0)
  
  ! allocate(cbst(dall), stat=checkstat)
  ! do i = 1,dall
  !   cbst(i) = obst(i) - dsyn(i)
  ! enddo
  ! mean = sum(cbst(1:dall))/dall
  ...
  if(ifsyn == 1) then
    ...
    do k=1,nz
      do j=1,ny
        do i=1,nx
          write(65,'(5f10.4)') gozd+(j-2)*dvzd,goxd-(i-2)*dvxd,depz(k),vsftrue(i,j,k)
          write(63,'(5f10.4)') gozd+(j-2)*dvzd,goxd-(i-2)*dvxd,depz(k),vsf(i,j,k)
        enddo
      enddo
    enddo
    close(65)
